...
const Product = ({product}) => {
    const {productId, name, description, image, rating, reviews, quantity, instock, price} = product;

    const { addToCart } = useContext(StateContext)

  return (
    <React.Fragment>
        <Link className="col-lg-3 col-sm-6" to={"/product/" + productId}>
        
            <div className="item" id={productId}>
